// Ramps concurrent VUs listing chats via the la-suite Conversations REST API (OIDC Bearer access token,
// fetched in setup()). Conversations is a Django/DRF app over Postgres; this read path is CPU + DB-bound. The
...
export default function (data: { token: string }): void {
  iterationStart();
  const res = http.get(`${BASE_URL}/api/v1.0/chats/?page_size=1`, {
